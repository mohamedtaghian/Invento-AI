...
- If the UI also has an advisor **settings screen** (schedule toggle, email toggle, send hour, timezone, location, lead time) — locate it the same way. If it doesn't exist yet, flag it; §8 below is read-focused since no save/update endpoint has been provided.
...
- A missing insight type (e.g. no weather line) is expected — just render whatever `insights` contains, nothing to flag. (If it's specifically the weather line that's always missing, that may be explained by Advisor Settings — see §8.)
...
- **This works even when `Advisor Settings.isEnabled` (§8) is `false`** — that flag only stops the _scheduled_ brief; the manual button is confirmed to bypass it. Don't gate this button on that setting client-side.
...
## 8. Endpoint F — `GET /advisor/settings` (advisor configuration screen)
...
export interface AdvisorSettings {
  ...
  countryCode: string | null; // ISO 3166-1 alpha-2, uppercase — picks local calendar events (Ramadan + both Eids always included regardless)
...
**Rules specific to this endpoint — several fields have a sharp edge:**

1. **`timezone` vs `effectiveTimezone` are not interchangeable.** Bind any timezone _editor/picker_ to the raw `timezone` field (nullable) — never prefill or save `effectiveTimezone` into the stored value, or you'll silently persist a default the owner never actually chose. Use `effectiveTimezone` only for _display_ copy elsewhere, e.g. "your brief arrives at 7:00 ({effectiveTimezone})", since it's guaranteed non-null.
2. **`hasWeatherLocation: false` means no weather insight will ever appear and no outbound weather request is made at all** — this is not a bug or a missing section, it's the direct, deterministic reason. Render "add a location to get weather advice" (with a way to set `city`/coordinates) rather than treating it as an empty/failed section. This is also the explanation if you noticed in §3 that weather insights never show up for a store.
3. **`isEnabled: false` only disables the automatic _scheduled_ generation** — it has no effect on the manual "generate now" button (Endpoint D). Don't let a settings toggle for this accidentally disable or hide that button elsewhere in the UI.
4. **`emailEnabled` only controls the owner's copy** — call this out in the UI copy (e.g. a caption under the toggle) so an admin user looking at this screen doesn't assume it affects their own inbox, since admins never receive the email regardless.
5. `leadTimeDays` is currently store-wide, not per-supplier or per-product — phrase any helper text accordingly (e.g. don't imply it can vary by item yet).
6. `countryCode` affects which calendar-based `seasonal_event` insights can appear, except Ramadan and the two Eids, which show regardless of country.

**This is a read-only contract.** No update/save endpoint (`PUT`/`PATCH /advisor/settings` or similar) has been provided. If the settings screen has editable form controls:

- Wire the form to **prefill** from this `GET` response.
- Do **not** guess a save endpoint's method, path, or body shape.
- Flag clearly that the save action needs its own confirmed contract before it can be wired — same rule as any other undocumented endpoint in this task.

**States:** Loading · Loaded (prefill form/display) · 401/403. No empty state — the lazy-create means it's always a normal `200`.
...
- [ ] The settings screen (if present) calls `GET /advisor/settings`, prefills its form/display from the response, binds any timezone editor to `timezone` (never `effectiveTimezone`), and does not attempt to call an unconfirmed save endpoint.
...
- [ ] Empty, stale, loading, loaded, not-found, and cooldown states all render correctly across all six endpoints.
