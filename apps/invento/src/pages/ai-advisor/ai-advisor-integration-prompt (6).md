...
- If the UI also has an advisor **settings screen** (schedule toggle, email toggle, send hour, timezone, location, lead time) — locate it the same way. Its API is now fully specified (§8), read and write both.
...
## 2. Shared types (used across the brief/insight endpoints — A, C, D, E)
...
export interface RestockPayload {
  ...
  leadTimeDays: number; // the store's Advisor Settings leadTimeDays at generation time — see §8
}
...
- A missing insight type (e.g. no weather line) is expected — just render whatever `insights` contains, nothing to flag. (If it's specifically the weather line that's always missing, that's explained by `hasWeatherLocation` in Advisor Settings — see §8.)
...
- **This works even when `Advisor Settings.isEnabled` (§8) is `false`** — that flag only stops the _scheduled_ brief; the manual button bypasses it. Don't gate this button on that setting client-side.
...
## 8. Endpoint F — Advisor Settings (`GET` / `PATCH /advisor/settings`)

### Shared type

```typescript
export interface AdvisorSettings {
  isEnabled: boolean; // false stops the SCHEDULED brief only — POST /advisor/generate (§6) still works regardless
  emailEnabled: boolean; // whether the brief is also emailed to the store owner — admins never receive it, regardless of this flag
  sendHour: number; // 0–23, on the store's OWN wall clock
  timezone: string | null; // what is actually STORED — null if the owner never explicitly chose one (falls back to platform default)
  effectiveTimezone: string; // what the scheduler will actually use (stored zone, or platform default) — never null; GET-only display field, not sent on write
  countryCode: string | null; // ISO 3166-1 alpha-2 — picks local calendar events (Ramadan + both Eids always included regardless)
  city: string | null; // display only — the forecast itself is fetched from lat/lng below
  latitude: number | null;
  longitude: number | null;
  hasWeatherLocation: boolean; // true exactly when BOTH latitude and longitude are set; GET-only derived field, not sent on write
  leadTimeDays: number; // store-wide supplier lead time; what a restock recommendation (§2 RestockPayload.leadTimeDays) has to beat
  updatedAt: string;
}
```

### `GET /advisor/settings` — read
...
- Same auth rules as the others: `OWNER`/`ADMIN` only, **401**/**403** as usual.
- **200 — always.** The settings row is created lazily on first read with sensible defaults, so a store that has never opened this screen still gets a normal response, not an empty/404 state.
- Response: full `AdvisorSettings`.

### `PATCH /advisor/settings` — update

```
PATCH /advisor/settings
Authorization: Bearer <token>   (required)
Body: partial — every field optional, patch one switch at a time
```
...
export interface UpdateAdvisorSettingsRequest {
  ...
  // NEVER send `storeId` — the server derives it from the token; any undeclared field (storeId included) is a 400
}
```

- **200** → the full **merged** `AdvisorSettings` — fields you didn't include in the request are left untouched server-side. **Replace your local settings state entirely with this response** (it's the canonical merged result); don't hand-merge the request body into local state yourself.
- **401** — missing/invalid token.
- **403** — `USER` token.
- **400** on any of:
  - `sendHour` outside 0–23
  - `timezone` not a real IANA zone (e.g. `"Mars/Olympus"`) — validate client-side against a known zone list if the picker allows free text, but the server is authoritative
  - `countryCode` not exactly 2 letters
  - `leadTimeDays` outside 0–120
  - `latitude`/`longitude` out of range
  - **`latitude` and `longitude` sent one without the other** — checked against the row _after_ merging with what's already stored, so sending only `latitude` when a `longitude` already exists on the store is fine; sending only `latitude` when none exists yet is a 400
  - any undeclared field, including `storeId`

**Rules specific to this endpoint — these directly shape the settings form's save behavior:**

1. **Send only the fields the user actually changed** (a true partial patch), not the whole form state every time — this also naturally avoids ever accidentally including something like `storeId`. The four request examples in the spec (schedule-only, location-only, clear-location, switch-off) suggest the settings screen is naturally sectioned (schedule, location/weather, notifications, lead time) with an independent save per section, rather than one big form-wide submit — build it that way if the existing UI supports it.
2. **To turn weather off, clear the location by sending both `latitude: null` and `longitude: null` in the same request.** A "clear location" control must always submit both together, even if the UI only shows one control for it (e.g. a single "remove location" button).
3. **To turn weather on**, send `latitude` and `longitude` together (plus optionally `countryCode`/`city`) in one request — sending just one coordinate when neither is currently set is a 400.
4. **`timezone` changes more than delivery time** — every measurement window the Advisor uses ("last 7 days", "today") is measured in that zone's calendar days, and the brief is generated when `sendHour` strikes in that zone. Worth a line of helper text near the timezone picker so the owner understands it's not purely cosmetic.
5. Map the specific 400 validation messages back to the right form field/section (lonely-coordinate error → location section; bad timezone → timezone field; unknown/undeclared-field error → treat as a client bug to fix, not a user-facing validation message, since a correctly-built form should never send an undeclared field).
6. After a successful save that changes `isEnabled` or the weather location, consider whether the dashboard panel (§3) or its weather-availability messaging needs to reflect the new state next time it loads — not a hard requirement to live-sync immediately, but don't leave stale copy contradicting the just-saved settings if both are visible together.

**States (GET):** Loading · Loaded (prefill form/display) · 401/403. No empty state — lazy-create means it's always a normal `200`.
**States (PATCH):** Idle · Saving (disable the section's own save control, not the whole form) · Saved (replace local state from response) · 400 (inline, mapped to the right field) · 401/403.

---

## 9. Acceptance checklist (verify before considering this done)
...
- [ ] The "generate now" button calls `POST /advisor/generate`, is disabled while in flight, swaps the panel's state from the response on success, and is never gated on `Advisor Settings.isEnabled`.
...
- [ ] A successful insight PATCH replaces just that one insight in the currently rendered array (immutably), without refetching the whole brief.
- [ ] Dismissing a `demand_gap` insight is not wired to any separate chat/message "reviewed" feature.
- [ ] The settings screen calls `GET /advisor/settings` to prefill and `PATCH /advisor/settings` to save, sending only changed fields per section rather than the full form state.
- [ ] Any timezone editor is bound to `timezone` (never `effectiveTimezone`); `effectiveTimezone` is used for display copy only and is never sent in a `PATCH` body.
- [ ] Clearing the weather location sends `latitude: null` and `longitude: null` together; setting it sends both coordinates together — never one alone.
- [ ] `storeId` (or any field outside `UpdateAdvisorSettingsRequest`) is never sent in the `PATCH /advisor/settings` body.
- [ ] The settings screen explains `hasWeatherLocation: false` as "no location set" rather than a broken/empty weather section, and clarifies `emailEnabled` only affects the owner's inbox.
- [ ] All response types match §2–§8 exactly, including the 7 discriminated `payload` shapes.
- [ ] Empty, stale, loading, loaded, not-found, and cooldown states all render correctly across all endpoints.
...
- [ ] 400 (B, C, E, and Settings PATCH), 401, 403, 404 (C and E), and 429 (D only) are handled using the app's existing error patterns, with Settings PATCH 400s mapped to the specific offending field.
...
- [ ] OnPush change detection still works correctly for all new reactive state, including per-insight and per-settings-section in-flight states.
